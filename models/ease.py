...
class Learner(BaseLearner):
    ...
    def save_times(self):
        ...
        fn = f'{root}/rebuttal-training_time-inc{inc}-{model_name}-{backbone_name}-{dataset_name}.npy'

        print('time used so far: ', self.time)
        np.save(fn, [self.time])
    ...
    def print_num_parameters(self):
        ...
        adapter_list = self._network.backbone.adapter_list
        ...
        for adapters in adapter_list:
            ...
            for adapter in adapters:
                count_per_task += sum( p.numel() for p in adapter.parameters())
    ...
    def incremental_train(self, data_manager):
        ...
        torch.cuda.synchronize()
        self.time += time.time() - train_start
